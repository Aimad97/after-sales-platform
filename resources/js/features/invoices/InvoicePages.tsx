...
export function InvoiceFormPage() {
    ...
    return (
        <section className="max-w-6xl space-y-6">
            ...
            {clients.length === 0 || products.length === 0 ? <p className="rounded-md bg-amber-50 p-4 text-sm text-amber-800">Create at least one client and one product before creating an invoice.</p> : null}
                    ...
                    {fields.map((field, index) => {
                        ...
                        return (
                            <div key={field.id} className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-2 xl:grid-cols-6">
                                <Field label="Product" error={itemError?.product_id?.message}><select className={inputClassName} value={watchedItems[index]?.product_id ?? 0} onChange={(event) => { const product = products.find((entry) => entry.id === Number(event.target.value)); form.setValue(`items.${index}.product_id`, Number(event.target.value), { shouldValidate: true }); if (product) form.setValue(`items.${index}.warranty_months`, product.default_warranty_months, { shouldValidate: true }); }}><option value={0}>Select product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} · {product.sku}{product.active ? '' : ' (inactive)'}</option>)}</select></Field>
