...
export function ReportsPage() {
    ...
    const [requestedExport, setRequestedExport] = useState<ReportExport | null>(null);
    ...
    const techniciansQuery = useQuery({
        queryKey: ['technicians', 'report-filters'],
        queryFn: () => listTechnicians({ per_page: 100, sort: 'employee_code', direction: 'asc' }),
    });
    const brandsQuery = useQuery({
        queryKey: ['catalog', 'brands', 'report-filters'],
        queryFn: () => listBrands({ per_page: 100, sort: 'name', direction: 'asc', active: '' }),
    });
    const categoriesQuery = useQuery({
        queryKey: ['catalog', 'categories', 'report-filters'],
        queryFn: () => listCategories({ per_page: 100, sort: 'name', direction: 'asc', active: '' }),
    });
    const productsQuery = useQuery({
        queryKey: ['catalog', 'products', 'report-filters'],
    ...
    const exportMutation = useMutation({
        ...
        onSuccess: (nextExport) => setRequestedExport(nextExport),
    });
    const exportStatusQuery = useQuery({
        queryKey: ['report-exports', requestedExport?.uuid],
        queryFn: () => getReportExport(requestedExport?.uuid ?? ''),
        enabled: requestedExport !== null,
        refetchInterval: (query) => {
            const currentExport = query.state.data;
            return currentExport && isTerminalExportStatus(currentExport.status) ? false : 2_500;
        },
    });
    const exportJob = exportStatusQuery.data ?? requestedExport;
    ...
    const columns = useMemo<DataTableColumn<ReportRow>[]>(
        () =>
            reportColumns(reportQuery.data?.columns ?? {}, reportQuery.data?.data ?? []).map(([key, label]) => ({
                id: key,
                header: label,
            ...
            <form className="rounded-xl border border-slate-200 bg-slate-50 p-4" onSubmit={submitFilters}>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                    <label className="text-sm font-medium text-slate-700">
                        <span>From date</span>
                        <input className={inputClassName} type="date" {...form.register('date_from')} />
                    </label>
                    <label className="text-sm font-medium text-slate-700">
                        <span>To date</span>
                    ...
                    <label className="text-sm font-medium text-slate-700">
                        ...
                        <select className={inputClassName} {...form.register('technician_id')}>
                    ...
                    {supportsStatus && (
                        <label className="text-sm font-medium text-slate-700">
                            <span>{reportType === 'repairs' ? 'Repair result' : 'Status'}</span>
                            <input className={inputClassName} list={statusListId} placeholder="All statuses" {...form.register('status')} />
                            <datalist id={statusListId}>
                                {statusSuggestions(reportType).map((status) => (
                    ...
                    <label className="text-sm font-medium text-slate-700">
                        <span>Brand</span>
                        <select className={inputClassName} {...form.register('brand_id')}>
                    ...
                    <label className="text-sm font-medium text-slate-700">
                        <span>Category</span>
                        <select className={inputClassName} {...form.register('category_id')}>
                    ...
                    <label className="text-sm font-medium text-slate-700">
                        <span>Product</span>
                        <select className={inputClassName} {...form.register('product_id')}>
                    ...
                    <label className="text-sm font-medium text-slate-700">
                        ...
                        <select className={inputClassName} {...form.register('warranty_state')}>
                            <option value="">All warranty states</option>
                            {warrantyStates.map((state) => (
                    ...
                    <label className="text-sm font-medium text-slate-700">
                        <span>Client</span>
                        <select className={inputClassName} {...form.register('client_id')}>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-600">
                            {reportQuery.isFetching
                                ? 'Refreshing results…'
                                : reportQuery.data
                                  ? `${reportQuery.data.meta.total} matching record${reportQuery.data.meta.total === 1 ? '' : 's'}`
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                        ...
                        >
                            {exportMutation.isPending ? 'Queueing export…' : 'Export CSV'}
                        </button>
                        {exportIsReady && (
                            <button
                                type="button"
                ...
                <p className="mt-2 text-xs text-slate-500">
                    Excel and PDF are not shown because no compatible server-side export package is installed.
                </p>
                {(exportMutation.error || downloadMutation.error || exportStatusQuery.error) && (
                    <div className="mt-4">
                        <ErrorMessage error={exportMutation.error ?? downloadMutation.error ?? exportStatusQuery.error} />
                    </div>
                )}
                {exportJob && (
                    <div
                        className={`mt-4 rounded-lg border p-3 text-sm ${exportJob.status === 'failed' ? 'border-rose-200 bg-rose-50 text-rose-800' : exportIsReady ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-blue-200 bg-blue-50 text-blue-800'}`}
                    >
                        <span className="font-semibold">Export {humanize(exportJob.status)}.</span> {exportMessage(exportJob)}
                    </div>
                )}
                <div className="mt-5">
